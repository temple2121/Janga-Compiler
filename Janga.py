...
subprocess.run(["clang", "-arch", "x86_64", "-w", "-o", 'Executables/'+executable_name, "out.o"]) #machine code into binary
if not 'input' in source:
	subprocess.run(["./Executables/"+executable_name])	#run that bad boy
else:
	subprocess.run([
    "open", "-a", "Terminal", "./Executables/"+executable_name])